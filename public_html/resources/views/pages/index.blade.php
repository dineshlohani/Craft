...
@section('title', 'CraftsManNepal-Cheapest With Best-Handicraft Manufacturer & Wholesale Supplier')
...
<div class="home1-slider swiper-container">
    <div class="swiper-wrapper">
        <div class="home1-slide-item swiper-slide" data-swiper-autoplay="7000" data-bg-image="{{asset('public/frontend/assets/images/slider/home1/banner1.jpg')}}">
            <div class="home1-slide1-content">
                <span class="bg"></span>
                <span class="slide-border"></span>
                <span class="icon"><img src="{{asset('public/frontend/assets/images/slider/home1/slide-1-1.png')}}" alt="Slide Icon" style="margin-top: -40px;"></span>
                <h2 class="title">Welcome To</h2>
                <h2 class="title">Crafts Man Nepal</h2>
                <h5 class="sub-title">Delivering Top Notch Handicraft Products</h5>
                <div class="link"><a href="{{ url('all/products') }}">shop now</a></div>
            </div>
        </div>
        @foreach($banner as $row)
            <div class="home1-slide-item swiper-slide" data-swiper-autoplay="5000" data-bg-image="{{asset($row->image)}}">
                <div class="home1-slide3-content">
                    <span class="bg" data-bg-image="{{asset('public/frontend/assets/images/slider/home1/slide-2-1.png')}}"></span>
                    <span class="slide-border"></span>
                    <span class="icon">
                        <img src="{{asset('public/frontend/assets/images/slider/home1/slide-2-2.png')}}" alt="Slide Icon">
                        <img src="{{asset('public/frontend/assets/images/slider/home1/slide-2-3.png')}}" alt="Slide Icon">
                    </span>
                    <h2 class="title" style="color: white;">{{ $row->title }}</h2>
                    <h3 class="sub-title" style="color: white;">{{ $row->sub_title }}</h3>
                    <div class="link"><a href="{{ url('all/products') }}">shop now</a></div>
                </div>
            </div>
        @endforeach
        ...
        <div class="category-banner1-carousel">
            ...
            <div class="col">
                <div class="category-banner1">
                    <div class="inner">
                        <a href="{{ url('categoryproduct/'.$key->id) }}" class="image"><img src="{{asset($key->category_image)}}" alt=""></a>
                        <div class="content">
                            <h3 class="title">
                                <a href="{{ url('categoryproduct/'.$key->id) }}">{{ $key->category_name }}</a>
                ...
                <div class="col">
                    <div class="product">
                        ...
                        @if($row->discount_price == NULL)
                            ...
                            <a href="{{ url('product/details/'.$row->id.'/'.$row->product_name) }}" class="image">
                                <img src="{{ URL::to('public/media/product/'.json_decode($row->filename, true)[0]) }}" alt="Product Image" style="">
                                <img class="image-hover " src="{{ URL::to('public/media/product/'.json_decode($row->filename, true)[1]) }}" alt="Product Image">
                            </a>
                            @guest()
                                <a href="#offcanvas-wishlist" class="add-to-wishlist hintT-left addwishlist" data-id="{{ $row->id }}" data-hint="Add to wishlist"><i class="far fa-heart"></i></a>
                            @else
                                <a href="#offcanvas-wishlist" class="add-to-wishlist hintT-left offcanvas-toggle addwishlist" data-id="{{ $row->id }}" data-hint="Add to wishlist"><i class="far fa-heart"></i></a>
                            @endguest
                        </div>
                        <div class="product-info">
                            <h6 class="title"><a href="{{ url('product/details/'.$row->id.'/'.$row->product_name) }}">{{ $row->product_name }}</a></h6>
                            @if($row->discount_price == NULL)
                                <span class="price">${{$row->selling_price}}</span>
                            @else
                        <span class="price">
                            <span class="old">${{$row->selling_price}}</span>
                            <span class="new">${{$row->discount_price}}</span>
                        </span>
                            @endif
                            <div class="product-buttons">
                                <a href="#quickViewModal" data-toggle="modal" class="product-button hintT-top" data-hint="Add To Cart" id="{{ $row->id }}" onclick="productview(this.id)"><i class="fal fa-shopping-cart"></i></a>
                            </div>
                            ...
                            <div class="product-stock-status">
                                ...
                                <span class="sold">Available: <span>{{ $row->product_quantity }}</span></span>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <div class="container">
                ...
                <div class="col">
                    <div class="product">
                        ...
                        @if($row->discount_price == NULL)
                            ...
                            <a href="{{ url('product/details/'.$row->id.'/'.$row->product_name) }}" class="image">
                                <img src="{{ URL::to('public/media/product/'.json_decode($row->filename, true)[0]) }}" alt="Product Image">
                                <img class="image-hover " src="{{ URL::to('public/media/product/'.json_decode($row->filename, true)[1]) }}" alt="Product Image">
                            </a>
                            @guest()
                                <a href="#offcanvas-wishlist" class="add-to-wishlist hintT-left addwishlist" data-id="{{ $row->id }}" data-hint="Add to wishlist"><i class="far fa-heart"></i></a>
        ...
        <div class="row learts-mt-50">
            <div class="col text-center">
                <a href="{{ url('all/products') }}" class="btn btn-dark btn-hover-primary">Go To Shop</a>
            </div>
        </div>
    ...
    <div class="container">
        ...
        <div class="section-title2 text-md-left text-center row justify-content-between align-items-center">
            <div class="col-md-auto col-12">
                <!-- Section Title Start -->
                <h2 class="title title-icon-right">{{$fcat->category_name}}</h2>
                <!-- Section Title End -->
            </div>
            <div class="col-md-auto d-none d-md-block mt-4 mt-md-0">
                <a href="{{ url('categoryproduct/'.$fcat->id) }}" class="btn btn-dark btn-hover-primary">View All</a>
            </div>
        </div>
                ...
                <div class="col-lg-6 col-12 overflow-hidden learts-mb-30">
                    <div class="product-summery customScroll">
                        ...
                        <form id="ajaxformcart1">@csrf
                            ...
                            <div class="product-buttons">
{{--                                <button href="" class="btn btn-dark btn-outline-hover-dark"><i class="fal fa-search"></i> View Details</button>--}}
